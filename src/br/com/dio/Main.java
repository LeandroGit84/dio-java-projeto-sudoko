...
public class Main {
    ...
    private static void startGame(final Map<String, String> positions) {
        if (positions.isEmpty()) {
            throw new IllegalArgumentException("Nenhuma configuração de Sudoku foi passada.");
        }
    ...
    private static void inputNumber() {
        ...
        var row = runUntilGetValedNumber(0, 8);
        System.out.println("Informe a coluna em que o numero sera inserido: ");
        var col = runUntilGetValedNumber(0, 8);
        System.out.printf("Informe o numero que vai entrar na posição [L - %s, C - %s]\n", row, col);
        var value = runUntilGetValedNumber(1, 9);
    ...
    private static void removeNumber() {
        ...
        System.out.println("Informe a 'linha' em que o numero sera alterado: ");
        var row = runUntilGetValedNumber(0, 8);
        System.out.println("Informe a 'coluna' em que o numero sera alterado: ");
        var col = runUntilGetValedNumber(0, 8);
        System.out.printf("Informe o numero que vai entrar na posição [L - %s, C - %s]\n", row, col);

        if (!board.clearValue(row, col)) {
            System.out.printf("A posição [L - %s, C - %s] tem um valor fixo. \n", row, col);
        }
    ...
    private static void showCurrentGame() {
        ...
        for (var row : board.getSpaces()) {
            for (var space : row) {
                ...
                var value = space.getActual();

                values[pos++] = (isNull(value)) ? "  " : " " + value;
            }
        }

        System.out.printf(BoardTemplate.BOARD_TEMPLATE, values);
    }
    ...
    private static int runUntilGetValedNumber(final int min, final int max) {
        var current = scanner.nextInt();
        while (current < min || current > max) {
            System.out.printf("Informe um numero entre o %s e o %s\n", min, max);
